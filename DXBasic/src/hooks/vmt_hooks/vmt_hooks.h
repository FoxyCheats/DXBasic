...
class vft_FunctionData {
public:
	vft_FunctionData(void* fn, size_t fnIdx) : m_function(fn), m_functionIndex(fnIdx) {}
public:
	auto getFunction() {
...
class VFT {
public:
	VFT(vft_FunctionData* vft, size_t vftSize) : m_vft(vft), m_vftSize(vftSize) {}
	VFT(std::vector<vft_FunctionData> vft) : m_vft(vft.data()), m_vftSize(vft.size()) {}
	VFT(void* vft, size_t vftSize) : m_vft((vft_FunctionData*)vft), m_vftSize(vftSize) {}
public:
	auto getVft() {
		return m_vft;
	}
...
public:
	void hook(vft_FunctionData data) {
